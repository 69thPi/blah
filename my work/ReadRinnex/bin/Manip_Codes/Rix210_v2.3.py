...
if not Fldr_Gc.exists():
    Fldr_Gc.mkdir()
    print("Path Created!")

end=len(Nav_files)
for cnt in range(0,end):
    fl_nm = Nav_files[cnt][:-4]+'-gc.txt' #new target name ext = -gc.txt
    source = Path(Fldr_Gc)/fl_nm
    target = open(str(source), 'w')
    ...
    target.close()
print("Conversion completed! \nPls check ../GPS_Nav_Dat/GC for Geocentric Co-Ordinate file(s).")
