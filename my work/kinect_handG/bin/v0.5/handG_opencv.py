...
while(True):
    ...
    if(finger_count!=0):
        ...
        if k==27:
            break
        if k==ord('0'):
            cv.imwrite("train/zero/%d.jpg"%c,frame)
            c+=1
        if k==ord('1'):
            cv.imwrite("train/one/%d.jpg"%c,frame)
            c+=1
        if k==ord('2'):
            cv.imwrite("train/two/%d.jpg"%c,frame)
            c+=1
        if k==ord('3'):
            cv.imwrite("train/three/%d.jpg"%c,frame)
            c+=1
        if k==ord('4'):
            cv.imwrite("train/four/%d.jpg"%c,frame)
            c+=1
        if k==ord('5'):
            cv.imwrite("train/five/%d.jpg"%c,frame)
            c+=1
        if k==ord('f'):
            cv.imwrite("test/none/%d.jpg"%c,frame)
            c+=1
